    ...
    async confirmCapture(input) {
      ...
      if (input.decision === "update" && memory) {
        memory = {
          ...
          confidenceState: "confidenceState" in input ? input.confidenceState ?? null : memory.confidenceState,
          provenance: "provenance" in input ? input.provenance ?? null : memory.provenance,
          reviewAfter: "reviewAfter" in input ? input.reviewAfter ?? null : memory.reviewAfter,
          expiresAt: "expiresAt" in input ? input.expiresAt ?? null : memory.expiresAt,
