...
`1.0.0` matches the current release. Future packaging uses the actual shared
...
`@nuzo/memory@1.0.0` package. The release gate installs the artifact through
...
```bash
codex mcp add nuzo -- npm exec --yes --package=@nuzo/memory@1.0.0 -- nuzo-mcp-server
```
