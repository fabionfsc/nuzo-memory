...
`1.1.0` matches the current release. Future packaging pins the actual plugin
...
`@nuzo/memory@1.1.0` package. The release gate validates the artifact with the
...
```bash
claude mcp add --transport stdio nuzo -- npm exec --yes --package=@nuzo/memory@1.1.0 -- nuzo-mcp-server
```
