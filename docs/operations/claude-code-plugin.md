...
```json
{
  "mcpServers": {
    "nuzo": {
      "command": "npm",
      "args": ["exec", "--yes", "--package=@nuzo/memory@0.3.1", "--", "nuzo-mcp-server"],
...
`0.3.1` matches the current release. Future packaging pins the actual plugin
...
The generated `0.3.1` config resolves the matching public
`@nuzo/memory@0.3.1` package. It has been installed through an isolated
